...
function ReferenceLink({
  ...
  useEffect(() => {
    ...
    if (matchedItem.type === "Writer") {
      ...
      if (
        matchedItem.type.toLowerCase() === "writer" &&
        matchedItem.type !== "Writer"
      ) {
          ...
          <ActionIcon
            ...
            onClick={async (e: React.MouseEvent) => {
              ...
              try {
                ...
                LogInfo(
                  `Received inline TTS result, cached: ${result.cached}, error: ${result.error || "none"}`,
                );
                ...
                if (result.error) {
                  notifications.update({
                    ...
                    loading: false,
                    autoClose:
                      result.errorType === "missing_key" ? false : 5000,
                ...
                LogInfo(
                  `Converted inline quote to Uint8Array, length: ${uint8Array.length}`,
                );
                ...
                audio.onerror = (e) => {
                  LogError(
                    `Inline quote audio playback error: ${JSON.stringify(e)}`,
                  );
                  notifications.show({
...
export function DefinitionRenderer({
  ...
  const renderTextWithLinks = (
    ...
    while ((match = regex.exec(segmentText)) !== null) {
      ...
      if (match.index > lastIndex) {
        parts.push(
          <span key={`text-${startKey}-${keyCounter++}`}>
      ...
      if (matchedItem) {
        ...
        parts.push(
          <span
            key={`missing-${startKey}-${keyCounter++}`}
            style={{
              color: "#999",
              fontStyle: "italic",
              fontWeight: 600,
    ...
    if (lastIndex < segmentText.length) {
      parts.push(
        <span key={`text-${startKey}-${keyCounter++}`}>
          {segmentText.substring(lastIndex)}
        </span>,
      );
    }
  ...
  const processTextForLinks = (segmentText: string, startKey: number) => {
    ...
    if (lastIndex < segmentText.length) {
      parts.push(
        <span key={`text-${key++}`}>{segmentText.substring(lastIndex)}</span>,
      );
    }
  ...
  segments.forEach((segment, idx) => {
    if (segment.type === "quote") {
      ...
      const { parts, nextKey } = processTextForLinks(
        segment.content,
        globalKey,
      );
      globalKey = nextKey;
