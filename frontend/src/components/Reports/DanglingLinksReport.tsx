...
export function DanglingLinksReport() {
  ...
  const handleDeleteLink = async (linkId: number, sourceWord: string) => {
    LogInfo(`[DanglingLinksReport] Deleting link: linkId=${linkId}, sourceWord=${sourceWord}`)
    ...
    try {
      ...
      LogInfo(`[DanglingLinksReport] Failed to delete link: ${error instanceof Error ? error.message : String(error)}`)
    ...
    <Stack gap="md">
      ...
      {!isLoading && danglingLinks && danglingLinks.length === 0 && (
        <Alert color="green" icon={<AlertTriangle size={20} />}>
          ...
          <Text size="sm">All links in your database point to valid items.</Text>
        </Alert>
      )}
      ...
      {!isLoading && danglingLinks && danglingLinks.length > 0 && (
        <>
          <Alert color="red" icon={<AlertTriangle size={20} />}>
            ...
            <Text size="sm">
              These links point to items that no longer exist, likely due to database corruption during deletion.
            </Text>
          </Alert>
                ...
                <Table.Tr key={link.linkId}>
                  ...
                  <Table.Td>
                    {link.missingSide === 'destination' ? (
                      <Anchor component={Link} to={`/item/${link.sourceItemId}?tab=detail`} fw={600}>
                        {link.sourceWord}
                      </Anchor>
                    ) : (
                      <Text c="dimmed" fs="italic">Missing Source ({link.sourceItemId})</Text>
                    )}
                  </Table.Td>
                  <Table.Td>
                  ...
                  <Table.Td>
                    <Badge color="red" variant="light">{link.missingSide}</Badge>
                  </Table.Td>
                  <Table.Td style={{ textAlign: 'right' }}>
                    <Button 
                      ...
                      onClick={() => handleDeleteLink(link.linkId, link.sourceWord)}
                    >
                      Delete Link
                    </Button>
                  </Table.Td>
