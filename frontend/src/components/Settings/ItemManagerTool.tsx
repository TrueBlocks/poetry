...
import { TextInput, Button, Stack, Group, Text, Table, ScrollArea, Alert, Badge, Loader, Paper, Select, Combobox, useCombobox, Grid, Box } from '@mantine/core'
import { SearchItemsWithOptions, UpdateItem, GetItem, GetSettings, UpdateSettings, DeleteItem } from '../../../wailsjs/go/main/App'
import { LogInfo, LogError } from '../../../wailsjs/runtime/runtime'
import { database } from '../../../wailsjs/go/models'
import { notifications } from '@mantine/notifications'
import { AlertCircle, Check, Search, ArrowRight, X } from 'lucide-react'

const ITEM_TYPES = ['Reference', 'Title', 'Writer', 'School', 'Term', 'Source', 'Cliche', 'Name']
...
export function ItemManagerTool() {
  ...
  const [targetItem, setTargetItem] = useState<database.Item | null>(null)
  ...
  useEffect(() => {
    GetSettings().then(settings => {
      if (settings) {
        const newSettings = { ...settings, managerOldType: oldType, managerNewType: newType }
  ...
  const handlePreview = async () => {
    ...
    setIsMerge(false)
    setConfirming(false)
    LogInfo(`Previewing rename: "${oldTitle}" (${oldType}) -> "${newTitle}" (${newType})`)
    ...
    try {
      ...
      if (titleResults && titleResults.length > 0) {
        ...
        const exact = titleResults.find(i => i.word === oldTitle && i.type === oldType)
      ...
      if (targetResults && targetResults.length > 0) {
        const exactTarget = targetResults.find(i => i.word === newTitle && i.type === newType)
        if (exactTarget) {
  ...
  const handleExecute = async () => {
    ...
    setIsExecuting(true)
    LogInfo(`Executing rename: "${oldTitle}" (${oldType}) -> "${newTitle}" (${newType})`)
    ...
    try {
      ...
      if (titleItem) {
        if (isMerge) {
           ...
           if (updatedTitleItem.definition) updatedTitleItem.definition = updatedTitleItem.definition.replace(regex, replacement)
           if (updatedTitleItem.derivation) updatedTitleItem.derivation = updatedTitleItem.derivation.replace(regex, replacement)
           if (updatedTitleItem.appendicies) updatedTitleItem.appendicies = updatedTitleItem.appendicies.replace(regex, replacement)
    ...
    <Paper p="md" withBorder>
      <Stack gap="md">
        ...
        <Text size="sm" c="dimmed">
          Rename items, change their type, or merge them. This tool will also update all references in other items.
        </Text>
        ...
        <Group justify="flex-end">
          ...
          >
            {confirming ? 'Click to Confirm' : (isMerge ? 'Execute Merge' : 'Execute Rename')}
          </Button>
        </Group>

        {executed && (
          <Alert icon={<Check size={16} />} title="Success" color="green">
            Successfully {isMerge ? 'merged' : 'renamed'} item and updated references.
          </Alert>
        )}

        {isMerge && !executed && (
          <Alert icon={<AlertCircle size={16} />} title="Merge Warning" color="yellow">
            Item "{newTitle}" already exists. Proceeding will DELETE "{oldTitle}" and update all references to point to "{newTitle}".
          </Alert>
        )}
          ...
          <Stack gap="xs">
            ...
            {titleItem && (
              <Paper p="xs" withBorder bg="var(--mantine-color-blue-0)">
                <Group>
                  <Badge>Source</Badge>
                  <Text size="sm">{titleItem.word} ({titleItem.type})</Text>
                  <ArrowRight size={16} />
                  <Text size="sm">{newTitle} ({newType})</Text>
              ...
              <Stack gap={4}>
                {affectedItems.map((item) => (
                  <Paper key={item.itemId} p="xs" withBorder>
                    <Group justify="space-between">
                      <Text size="sm" fw={500}>{item.word}</Text>
...
function TitleBadge({ count }: { count: number }) {
  return (
