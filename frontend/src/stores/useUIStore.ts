...
import { GetSettings, UpdateSettings, SaveLastWord } from '../../wailsjs/go/main/App'
    ...
    try {
      ...
      const state = {
        state: {
          ...
          outgoingCollapsed: s.collapsed?.outgoing !== undefined ? s.collapsed.outgoing : true,
          incomingCollapsed: s.collapsed?.incoming !== undefined ? s.collapsed.incoming : false,
  ...
  setItem: async (_name: string, value: string): Promise<void> => {
    try {
      ...
      if (!current.collapsed) {
        current.collapsed = { outgoing: true, incoming: false, linkIntegrity: false, itemHealth: false }
      }
...
export const useUIStore = create<UIState>()(
  persist(
    (set) => ({
      ...
      setOutgoingCollapsed: (collapsed) => set({ outgoingCollapsed: collapsed }),
      setIncomingCollapsed: (collapsed) => set({ incomingCollapsed: collapsed }),
