...
import { Container, Title, Button, TextInput, Select, Textarea, Group, Alert, Paper, Loader, Stack, Grid, Badge, Text, Box } from '@mantine/core'
import { notifications } from '@mantine/notifications'
import { GetItem, CreateItem, UpdateItem, SaveItemImage, GetItemImage, DeleteItemImage } from '../../wailsjs/go/main/App.js'
import { ArrowLeft, Save, AlertCircle, Check, X, Plus, Image as ImageIcon, Trash2 } from 'lucide-react'
import { useReferenceValidation } from '../hooks/useReferenceValidation'

export default function ItemEdit({ onSave, onCancel }: { onSave?: () => void; onCancel?: () => void } = {}) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    while ((match = tagRegex.exec(formData.definition)) !== null) {
      ...
      if (!['w', 'p', 't', 'word', 'writer', 'title'].includes(tagType)) {
        alert(`Invalid tag type found: "${tagType}". Allowed types are: w, p, t, word, writer, title.`)
        return
      }
    }
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      if ((e.metaKey || e.ctrlKey) && e.key === 's') {
        ...
        if (!formData.definition.trim() || formData.definition === 'MISSING DATA') {
          notifications.show({
            title: 'Validation Error',
            message: 'Definition field is required and cannot be "MISSING DATA"',
            color: 'red',
          })
          return
        }
  ...
  return (
    <Container size="lg">
      <Group justify="space-between" mb="xl">
        <Button component={Link} to={isNew ? '/' : `/item/${id}?tab=detail`} variant="subtle" leftSection={<ArrowLeft size={20} />}>
          Back
        </Button>
        <Title order={2}>{isNew ? 'New Item' : `Edit: ${item?.word || ''}`}</Title>
        <div />
      </Group>

      {saveMutation.isError && (
        <Alert icon={<AlertCircle size={20} />} title="Failed to save item" color="red" mb="md">
          {saveMutation.error?.message || String(saveMutation.error) || 'An unknown error occurred'}
        </Alert>
      )}
      ...
      <Paper shadow="sm" p="md" radius="md" withBorder>
        <form onSubmit={handleSubmit}>
          <Stack>
            <Grid>
              ...
              <Grid.Col span={2}>
                <Select
                  label="Type"
                  value={formData.type}
                  onChange={(value) => setFormData({ ...formData, type: value || 'Reference' })}
                  data={['Reference', 'Definition', 'Term', 'Concept', 'Title', 'Writer']}
                />
              </Grid.Col>
            </Grid>
            ...
            {formData.definition && (getExistingReferences().length > 0 || getMissingReferences().length > 0) && (
              <Alert 
              ...
              >
                <Stack gap="xs">
                  {getMissingReferences().length > 0 ? (
                    ...
                    <Group gap="xs">
                      ...
                      {getExistingReferences().map((result) => (
                        <Badge key={result.reference} color="green" size="sm" variant="light">
                          {result.reference}
                        </Badge>
                      ))}
                    </Group>
                  ...
                  {isValidating && (
                    <Text size="xs" c="dimmed">Validating references...</Text>
                  )}
                </Stack>
              </Alert>
            )}
            ...
            <TextInput
              ...
              onChange={(e) => setFormData({ ...formData, appendicies: e.target.value })}
            ...
            <Grid>
              ...
              <Grid.Col span={6}>
                <TextInput
                  ...
                  onChange={(e) => setFormData({ ...formData, sourcePg: e.target.value })}
