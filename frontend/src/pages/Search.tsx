...
const getFirstSentence = (text: string | null | undefined): string => {
  ...
  return match ? match[0] + '...' : (text.length > 100 ? text.substring(0, 100) + '...' : text)
}
...
export default function Search() {
  ...
  const { data: results, isLoading, error } = useQuery({
    queryKey: ['search', debouncedQuery, selectedTypes, selectedSource, useRegex, hasImage, hasTts, isSqlSearch],
    queryFn: () => {
      if (isSqlSearch) {
        return RunAdHocQuery(debouncedQuery)
      }
      // Use SearchItemsWithOptions if any filters are active or query is empty (wildcard)
      const hasFilters = selectedTypes.length > 0 || selectedSource || useRegex || hasImage || hasTts
    ...
    enabled: debouncedQuery.length > 0 || hasImage || hasTts || selectedTypes.length > 0 || !!selectedSource,
    staleTime: 30000, // Cache results for 30 seconds
  })

  const { exactMatches, wordMatches, otherResults, allResults } = useMemo(() => {
    if (!results || results.length === 0) {
      return { exactMatches: [], wordMatches: [], otherResults: [], allResults: [] }
    }
  ...
  }, [results, debouncedQuery, showAllResults])
  ...
  const linkQueries = useQueries({
    queries: (results || []).map((item: any) => ({
      ...
      queryFn: async () => {
        ...
        const outgoingLinks = links.filter((link: any) => link.sourceItemId === item.itemId)
  ...
  const itemWriters = useMemo(() => {
    ...
    results?.forEach((item: any, index: number) => {
      const writers = linkQueries[index]?.data?.filter((w): w is string => w !== null) || []
  ...
  const handleSaveSearch = async () => {
    if (!searchName || !query) {
      notifications.show({ title: 'Error', message: 'Name and query required', color: 'red' })
      return
    }
    try {
      ...
      setSearchName('')
      notifications.show({ title: 'Success', message: 'Search saved', color: 'green' })
    } catch (err) {
  ...
  const handleDeleteSavedSearch = async (name: string) => {
    try {
      ...
      const searches = await GetSavedSearches()
      setSavedSearches(searches || [])
      notifications.show({ title: 'Success', message: 'Search deleted', color: 'green' })
    } catch (err) {
      notifications.show({ title: 'Error', message: String(err), color: 'red' })
    }
  }
  ...
  useEffect(() => {
    const handleKeyDown = async (e: KeyboardEvent) => {
      ...
      if ((e.key === 'Delete' || e.key === 'Backspace') && combobox.dropdownOpened) {
        const selectedIndex = combobox.selectedOptionIndex
        LogInfo(`[Search] Delete/Backspace pressed, selectedIndex: ${selectedIndex}, filteredRecentSearches: ${JSON.stringify(filteredRecentSearches)}`)
        
        if (selectedIndex !== undefined && selectedIndex !== 'first-active' && selectedIndex >= 0 && selectedIndex < filteredRecentSearches.length) {
          e.preventDefault()
          const searchToDelete = filteredRecentSearches[selectedIndex]
          LogInfo(`[Search] Deleting recent search at index ${selectedIndex}: ${searchToDelete}`)
          
          await RemoveRecentSearch(searchToDelete)
          const searches = await GetRecentSearches()
          setRecentSearches(searches || [])
          
          LogInfo(`[Search] Recent search deleted, remaining: ${searches?.length || 0}`)
        } else {
          LogInfo(`[Search] Skipping delete - selectedIndex invalid: ${selectedIndex}`)
        }
      }
    }
    ...
    document.addEventListener('keydown', handleKeyDown)
    return () => document.removeEventListener('keydown', handleKeyDown)
  }, [combobox.dropdownOpened, combobox.selectedOptionIndex, filteredRecentSearches])
    ...
    <Container size="lg">
      ...
      <form onSubmit={handleSearch}>
        <Stack gap="xs" mb="xl">
          <Group align="flex-start" gap="md">
            ...
            <Accordion variant="contained" style={{ minWidth: '200px' }}>
              <Accordion.Item value="advanced">
                ...
                <Stack gap="md">
                  <div>
                    <Text size="sm" fw={500} mb="xs">Type Filters</Text>
                    <Group>
                      <Checkbox
                        label="Reference"
                        ...
                        onChange={(e) => {
                          if (e.currentTarget.checked) {
                            ...
                            setSelectedTypes(selectedTypes.filter(t => t !== 'Reference'))
                          }
                        }}
                      />
                      <Checkbox
                        ...
                        onChange={(e) => {
                          if (e.currentTarget.checked) {
                            ...
                            setSelectedTypes(selectedTypes.filter(t => t !== 'Writer'))
                          }
                        }}
                      />
                      <Checkbox
                        ...
                        onChange={(e) => {
                          if (e.currentTarget.checked) {
                            ...
                            setSelectedTypes(selectedTypes.filter(t => t !== 'Title'))
                          }
                        }}
                      />
                    </Group>
                  ...
                  <div>
                    <Text size="sm" fw={500} mb="xs">Media Filters</Text>
                    <Group>
                      <Checkbox
                        label="Has Image"
                        checked={hasImage}
                  ...
                  <div>
                    <Text size="sm" fw={500} mb="xs">Saved Searches</Text>
                    {savedSearches.length > 0 ? (
                      <Stack gap="xs">
                        {savedSearches.map((saved: any) => (
                          <Group key={saved.name} justify="space-between">
                            <Button variant="light" size="xs" onClick={() => handleLoadSavedSearch(saved)}>
                              {saved.name}
                            </Button>
                            <Button variant="subtle" size="xs" color="red" onClick={() => handleDeleteSavedSearch(saved.name)}>
                              <Trash2 size={14} />
                            </Button>
                          </Group>
                        ))}
                      ...
                      <Text size="sm" c="dimmed">No saved searches</Text>
                    )}
                    <Button leftSection={<Save size={16} />} size="xs" variant="outline" mt="xs" onClick={() => setSaveModalOpen(true)} disabled={!query}>
                      Save Current Search
                    </Button>
                  </div>
                </Stack>
          ...
          <Text size="sm" c="dimmed">
            Full-text search across words, definitions, and derivations. Supports AND, OR, NOT, and parentheses.
          </Text>
        </Stack>
      </form>

      <Modal opened={saveModalOpen} onClose={() => setSaveModalOpen(false)} title="Save Search">
        <Stack>
          <TextInput
            label="Search Name"
        ...
        <Stack>
          ...
          {isSqlSearch ? (
                      ...
                      <Stack gap="xs">
                        <Group justify="space-between" align="flex-start">
                          <Group align="flex-start" wrap="nowrap">
                            {item.type === 'Writer' && <SearchResultImage itemId={item.itemId} />}
                            <Title order={3} component={Link} to={`/item/${item.itemId}?tab=detail`} style={{ textDecoration: 'none', color: 'inherit' }}>
                              {item.word}
                            </Title>
                          </Group>
                      ...
                      <Stack gap="xs">
                        {Object.entries(row).map(([k, v]) => (
                          <Group key={k} align="flex-start">
                            <Text fw={700} size="sm" style={{ minWidth: 100 }}>{k}:</Text>
                            <Text size="sm" style={{ wordBreak: 'break-all' }}>{String(v)}</Text>
                          </Group>
                        ))}
                      </Stack>
                    )}
            ...
            <>
              {exactMatches.map((item: any, index: number) => {
                      ...
                      <div>
                        {item.definition ? (
                          <DefinitionRenderer 
                            ...
                            allItems={allItems}
                            stopAudio={() => {}}
                            currentAudioRef={audioRef}                          item={item}                          />
                        ) : (
                          <Text component="span">No definition</Text>
                        )}
                        {itemWriters[item.itemId]?.length > 0 && (
                          <Text component="span" size="sm" c="dimmed"> Writers: {itemWriters[item.itemId].join(', ')}</Text>
                        )}
                      </div>
                      <Group>
                ...
                {(wordMatches.length > 0 || otherResults.length > 0) && (
                  <Divider my="md" label="Word Matches" labelPosition="center" />
                )}
              </>
            )}
            ...
            {otherResults.map((item: any, index: number) => {
              const globalIndex = exactMatches.length + wordMatches.length + index
              const isSelected = selectedIndex === globalIndex
              return (
                <Paper
                  key={item.itemId}
      ...
      {allResults && allResults.length > 0 && (
        <Center mt="md">
          <Stack align="center" gap="xs">
            <Text size="sm" c="dimmed">
              Showing {allResults.length} of {(results as any)?.totalCount || allResults.length} results
            </Text>
      ...
      {debouncedQuery && results && results.length === 0 && !isLoading && (
        <Center p="xl">
          <Stack align="center">
            ...
            <Text size="sm" c="dimmed">Try a different search term or use fewer than 2 characters</Text>
          </Stack>
        </Center>
      )}
      ...
      {!debouncedQuery && (
        <Center p="xl">
          <Stack align="center">
            <SearchIcon size={48} />
            <Text size="lg" c="dimmed">Start typing to search</Text>
          </Stack>
        </Center>
      )}
    </Container>
